...
export async function middleware(request: NextRequest) {
  ...
  // Exception: allow fresh household creation (?fresh=1) to bypass check - dashboard will handle retries
  if (pathname.startsWith('/settings')) return response

  // If fresh=1 is set, skip the household check entirely - this is a new household creation
  // The dashboard page will retry with backoff to find the household_members record
  const isFresh = request.nextUrl.searchParams.get('fresh') === '1'
  if (isFresh && pathname === '/dashboard') return response
